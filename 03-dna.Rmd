...
- `table*()` : Summarize results from one of the `calculate*()` analyses. For example, `tableStuter()` summarized the results from `calculateStutter()`. 

For complete definitions and explanations of all functions available in `strvalidator`, please see the [`strvalidator` manual](https://www.thermofisher.com/order/catalog/product/4475073).  

## Drawing Conclusions

For each type validation analysis, heterozygote balance, stutter ratio, inter-locus balance, and stochastic threshold, there are recommended guidelines. In @strval, the authors recommend the referencing the following: 

- Heterozygote balance: @hill2011 for the specific PowerPlex® ESX 17 and ESI 17 systems and @gill1997 for the general recommendation of no less than 60%. 
- Stutter ratio: @hill2011, @westen2012, @brookes2012, and @tvedebrink2012 show greater stutter with more repeats. 
- Inter-locus balance: 

There are many other capabilities of `strvalidator` that we have not mentioned here for brevity. 
...
First, load the data, then slim it for analysis. Use `generateEPG()` to visualize an electropherogram-like plot of the data.  
