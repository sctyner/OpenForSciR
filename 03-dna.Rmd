...
```{r set1, fig.cap="Electropherogram-like `ggplot2` plot of the mean of all 8 samples in `set1`"}
...
dim(set1.slim)
head(set1.slim)
set1.slim %>% filter(Sample.Name != "Ladder") %>% 
generateEPG(kit = "ESX17") + 
  ggtitle("Mean peak heights for 8 samples from PC shown")
```
...
head(ref1.slim)
generateEPG(ref1.slim, kit = "ESX17") + ggtitle("True profile for sample PC") 
```
