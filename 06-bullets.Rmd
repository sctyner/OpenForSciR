...
#### Eric Hare, PhD; Heike Hofmann, PhD {-}

![Source: Close-up of a bullet under a Confocal Light Microscope in the Roy J Carver High-resolution microscopy lab. Photo by Heike Hofmann.](img/IMG_1480.png)
...
```{r, bullets-x3pcontain, echo=FALSE, fig.cap='An illustration of the internal structure of the x3p file format. x3p files contain an XML data file with metadata on the bullet scans, as well as binary data containing the surface topology measurements.'}
...
We will load the package and use some pre-provided x3p data to get a feel for the package functionality. We will work with the Center for Statistical Applications in Forensic Evidence (CSAFE) logo. In its original colored form, the logo looks like Figure \@ref(fig:bullets-csafelogo).

```{r, bullets-csafelogo, fig.cap='The CSAFE logo.'}
...
```{r, bullets-csafelogoscan, fig.cap='3D surface scan of the CSAFE logo.'}
...
Let's take a quick look at what we see on this bullet land (Figure \@ref(fig:bullets-b111)).

```{r, bullets-b111, fig.cap='Land 1 of Bullet 1 from Barrel 1 of the Hamby Study (Set 44).'}
...
`x3p_crosscut_optimize` looks for the first cross-section meeting this criteria, searching upwards from the base of the bullet land. With this value, we can extract and plot the cross-seciton like so (Figure \@ref(fig:bullets-cc)):
