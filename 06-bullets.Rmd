...
We will now walk through the process of performing a bullet match. We will take three bullet lands, with one pair that matches, and two pairs that do not match. 

```{r, echo=FALSE}
get_features <- function(res, g1_inc_x = diff(res$bullets$y)[1]) {
    lofX <- res$bullets
...
```{r}
bullet_temp <- tibble(
  Name = c("br411", "br423", "br424"),
  File = replicate(3, tempfile()),
  URL = c("https://tsapps.nist.gov/NRBTD/Studies/BulletMeasurement/DownloadMeasurement/e7ac7796-d98c-4c00-beab-2a74ec0dab17", "https://tsapps.nist.gov/NRBTD/Studies/BulletMeasurement/DownloadMeasurement/4227f885-60a6-4d3b-b4b2-307b9845415b", "https://tsapps.nist.gov/NRBTD/Studies/BulletMeasurement/DownloadMeasurement/f159930c-2d34-465d-9915-f1920bbd8039")
)

mapply(curl_download, bullet_temp$URL, bullet_temp$File)

bullets <- bullet_temp %>%
  rowwise() %>%
  mutate(Bullet = list(read_x3p(File)),
         BestCut = bulletCheckCrossCut(path = "", bullet = Bullet),
         CrossCut = list(get_crosscut(bullet = Bullet, x = BestCut)),
         Grooves = list(get_grooves(bullet = CrossCut)))

bullets_processed <- bullets %>%
  mutate(Processed = list(processBullets(bullet = Bullet,
                                         name = Name,
                                         x = BestCut,
                                         span = 0.75,
                                         grooves = Grooves$groove)),
         Smoothed = list(bulletSmooth(Processed) %>% filter(!is.na(l30))))

br411_br423 <- bulletGetMaxCMS(bullets_processed$Smoothed[bullets_processed$Name == "br411"][[1]], 
                        bullets_processed$Smoothed[bullets_processed$Name == "br423"][[1]],
                        column = "l30", 
                        span = 25)

br411_br424 <- bulletGetMaxCMS(bullets_processed$Smoothed[bullets_processed$Name == "br411"][[1]], 
                        bullets_processed$Smoothed[bullets_processed$Name == "br424"][[1]],
                        column = "l30", 
                        span = 25)

br423_br424 <- bulletGetMaxCMS(bullets_processed$Smoothed[bullets_processed$Name == "br423"][[1]], 
                        bullets_processed$Smoothed[bullets_processed$Name == "br424"][[1]],
                        column = "l30", 
                        span = 25)

rbind(
  get_features(br411_br423),
  get_features(br411_br424),
  get_features(br423_br424)
)
```
