...
<!-- Sam's question: what is the difference between striation & striae?-->

When a [bullet](#def:bullet) is fired from a [gun barrel](#def:gunbarrel), small imperfections in the barrel leave [striation](#def:striations) markings on the bullet. These markings are expressed most in the area of the bullet that has the closest contact to the barrel. 
...
<!-- Sam's question: is breech face actually on the projectile or no? What is a good definition of LEAs?  -->

Data on both [breech face](#def:breechface) and [land engraved areas](#def:leas) are available from the [NIST Ballistics Toolmark Research Database](https://tsapps.nist.gov/NRBTD/Studies/Search) (BTRD) in the [x3p](https://tsapps.nist.gov/NRBTD/Home/DataFormat) (XML 3-D Surface Profile) format. The x3p format was designed to implement a standard for exchanging 3D profile data. It was adopted by the Open Forensic [Metrology](#def:metrology) Consortium, or [OpenFMC](https://www.openfmc.org/), a group of firearm forensics researchers whose aim is to establish best practices for researchers using metrology in forensic science.
...
<!-- Sam's question: isn't Petraco the author of x3pr??-->
...
We can use the function `x3p_to_df` in order to convert this structure into a standard R data frame, which will allow us to do any number of data manipulation and plotting routines. In this case, Figure \@ref(fig:bullets-x3pplot) shows a simple scatterplot created with `ggplot2` of the height measurements across the surface of the bullet.
...
```{r, bullets-b111, fig.cap='Land 1 of Bullet 1 from Barrel 1 of the Hamby Study (Set 44). Source: [NRBTD](https://tsapps.nist.gov/NRBTD/Studies/BulletMeasurement/DownloadMeasurement/43567404-1611-4b40-ae74-a1e440e79f6a). (Rendered image has been down-sampled to speed up page load.)'}
knitr::include_url(url = "./img/br1b1l1/index.html", 
  height = "200px")
```

Immediately, we can clearly see the vertical striation markings. To better visualize these markings, we can extract a cross-section from the bullet and plot it in two dimensions. To accomplish this, `bulletxtrctr` provides us with a function `x3p_crosscut_optimize` to choose the ideal location at which to do so.
...
<!-- Sam's question: "percentage of pixels"??? this doesn't make sense to me --> 
...
<!-- Shouldn't it be y = 0? also "nearest the bottom" or Nearest the breech face instead of " nearest the barrel" ??  -->
1. The location should be nearest the barrel ($x = 0$) because the striation markings are most pronounced there.
2. Cross-sections taken near this location should be similar to this cross-section (stability).

The `x3p_crosscut_optimize` function looks for the first cross-section meeting this criteria, searching upwards from the base of the bullet land. With this value, we can extract and plot the cross-seciton like so (Figure \@ref(fig:bullets-cc)):
...
Most of the scans exhibit the pattern that we see here, where there are "wedges" on the left and right side. The wedge area is called the **shoulder**, and it is the area separating the land engraved area (the curved region in the middle) from the groove (the area not scanned because it doesn't exhibit striations). In other words, to better hone in on the striation markings along the land, we should subset this region to include only the middle curved land engraved area portion. Fortunately, `bulletxtrctr` provides us with functionality to automatically do that. First, we use the `cc_locate_grooves` function to detect the location of the grooves. This returns a list object, with one element being the two locations along the axis, and the lother element being the plot, given in Figure \@ref(fig:bullets-grooveloc).
...
With the grooves detected, we can now smooth out the surface using a LOESS [@cleveland:1979]. Once we do so, we obtain what we call a **bullet signature**, Figure \@ref(fig:bullets-loess), representing the clearest picture yet of the striation markings along the surface of the land.
...
We extract the ideal cross-sections from all 12 bullet lands, which allows us to plot Figure \@ref(fig:bullets-cscrosscut). We see a rather standard curved pattern, with well defined and a pronounced shoulders indicating the cutoff location for extracting the land.

```{r, bullets-cscrosscut, fig.cap='Ideal cross-sections for all 12 bullet lands.'}
bullets <- bullets %>% 
...
Next, with each of these profiles, we need to extract the grooves. In Figure \@ref(fig:bullets-csgrooves), we can see that the groove locations of the 12 bullet lands appear to be detected acceptably well, such that the middle portion between the two vertical blue lines represents a good sample of the land-engraved area.

```{r, bullets-csgrooves, fig.cap='Groove locations of each of the 12 bullet lands.'}
...
With the groove locations detected, we proceed as before by using a Loess in order to smooth out the curvature of the surface and focus on the striation markings. Figure \@ref(fig:bullets-cssigs) shows us the raw signatures of the 12 lands. The striation markings are much more visible now.

```{r, bullets-cssigs, warning=FALSE, fig.cap='Signatures for the 12 bullet lands.'}
bullets <- bullets %>% mutate(
  sigs = purrr::map2(
    .x = ccdata, .y = grooves, 
    .f = function(x, y) {
...
```{r, bullets-csalign, warning=FALSE, fig.cap='Alignment of two bullet lands (2-4 & 1-2)'}
...
Though the visual evidence is strong, we want to quantify the similarity. To do this, we're going to use a number of functions which extract features from the aligned signatures of the bullets. We'll extract the Cross-Correlation, the matching striation count, the non-matching striation count, and many more.
...
We are now ready to begin matching the bullets. We'll start by looking at Figure \@ref(fig:bullets-cscompare), which aligns the two bullets by bullet land and colors each of the cells (comparisons) by the CCF value. Encouragingly, we see a diagonal pattern to the matrix appear, which is to be expected given the assumption that the bullet scans were rotated and stored in rotational order. Note that we're also comparing each land to itself in two of the four panels, which as expected exhibit the highest CCF for matches.
...
We can improve upon these results by using the Random Forest in order to assess the probability of a match. Figure \@ref(fig:bullets-csrf) displays the random forest score, or match probability, of each of the land-to-land comparisons. The results are even stronger than using pure CCF in this case.
...
Finally, we can visualize the accuracy of our comparisons by highlighting the cells in which they were in fact matches (same-source). Figure \@ref(fig:bullets-csss) shows this, indicating that for the comparison between the two bullets, a couple of the lands didn't exhibit a high match probability. With that said, given that the other four lands exhibited a strong probability, this is high evidence these bullets were in fact fired from the same barrel.

```{r, bullets-csss, fig.cap='All Land-to-Land Comparisons of the bullets, highlighting same-source lands.'}
bullet_scores <- bullet_scores %>% mutate(
  data = data %>% purrr::map(
    .f = function(d) {
