...
Let's find out how the algorithm with a function `match_print_subarea` does find the corresponding areas between two shoe impressions. We will explain the process how to find the corresponding areas for circle $q_1$ in shoe $K$. The algorithm first finds many candidate circles in shoe $K$.   
...
The Figure \@ref(fig:graphq1) shows the matching result. By fixing circle $q_1$ (blue pixel), we found the closest circle (red pixel) for circle $q_1$ in shoe $K$, using circle information in Table \@ref(tab:resulttable). The blue circle in $Q$ and red circle in $K$ look pretty similar. We will repeat this process for two more circles that we fix in $Q$. The function `match_print_subarea` basically repeats this process for three circles in shoe $Q$. 
...
Next, we run `match_print_subarea()` to findcircles in $K$ that most closely correspond to the circles in $Q$. The result is shown in Figure \@ref(fig:exKM). In $K_1$, the algorithm finds the closest circles which show the most overlap with each of the circles that we fixed in $Q$. 
