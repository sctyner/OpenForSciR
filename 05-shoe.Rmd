...
The resulting image from the EverOS scanner is very large, making comparisons of images very time-consuming. To speed up the comparison process, we use only part of the image.  First, we used [MATLAB](https://www.mathworks.com/products/matlab.html) to downsample all of the images at 20%.   Next, we apply [edge detection](https://en.wikipedia.org/wiki/Edge_detection) to the images, and we end up with comparisons of the outlines of important patterns of shoe outsoles. We extract the edges of the outsole impression using the [Prewitt operator](https://en.wikipedia.org/wiki/Prewitt_operator) to obtain the ($x$,$y$) coordinates of the shoe patterns. By detecting the boundaries of the sole patterns, edges will contain class, subclass and possible RACs all together as our information of shoe impressions.   
...
We use the data CSAFE collected to construct known mated (KM) comparisons and known non-mated (KNM) comparisons. The KMs are two repeated impressions from the same shoe source, while the KNMs are from two different shoes. The shoes are identical in size, style, and brand: the only difference is who wore the shoes. We want to train a comparison method in this way because shoes that are the same size, style, and brand are the most similar to each other, and thus the detection of differences will be very hard. By training a method on the hardest comparisons, we make all comparisons feasible. 
...
We'll also use the other packages below: 
...
To find the closest areas of $q_1$, $q_2$, and $q_3$ in shoe $K$, we use the function `match_print`.
```{r match, fig.align = 'center', eval=FALSE}
match_print(shoeQ, shoeK, input_circ)
```
...
The function `match_print` will produce the best matching areas in shoe $K$ for circles of $q_1$, $q_2$, and $q_3$ in shoe $Q$, which we fixed. Figure \@ref(fig:matchgraph) shows the final result from the function `match_print`. Three circles of red, orange, green colors in the right panel in Figure \@ref(fig:matchgraph)indicate that those circles that the algorithm foound showed the best overlap with circles of $q_1$, $q_2$, and $q_3$ in shoe $Q$. 


Let's find out how the algorithm with a function `match_print` does find the corresponding areas between two shoe impressions. We will explain the process how to find the corresponding areas for circle $q_1$ in shoe $K$. The algorithm first finds many candidate circles in shoe $K$.   
...
In this section, we will show the example matching between circle $q_1$ and circle $k_8$ in Figure \@ref(fig:step1plot). We selected the circle $k_8$ on purpose to be matched with the circle $q_1$ well.  
...
A function `boosted_clique` is finding the set of sub-pixels that can be used for geomeric arrangement between circle $q_1$ and circle $k_8$. Using corresponding sub-pixels, the function will compute rotation angle and translation metric to result the best alignment between them. Circle $q_1$ is transformed to be on top of the circle $k_8$, using the calculated alignment information. The function `boosted_clique` will produce a resulting plot in Figure \@ref(fig:MCresult) and a table of similarity features in Table \@ref(tab:resulttable). 
...
Figure \@ref(fig:MCresult) have four sections. In the first row and first column, it showed distances between sub-pixels that we used for aligning of two circles. These two graphs are to check whether the found sub-pixels are actually good enough to be used for the geometric arrangement for two circles. For sub-pixels to be reasonable set of points for geometric arrangement, distances among those pixels should be aligned through the diagonal line in the left panel. In the first row and second column, it shows sub-pixels where blue pixels are in circle $q_1$ and red pixels are in circle $k_8$, after aligning them. The red point and blue point in this plot should be overlapped well, if they are corresponding points with the same geometries.  
...
The Figure \@ref(fig:graphq1) shows the matching result. By fixing circle $q_1$ (blue pixel), we found the closest circle (red pixel) for circle $q_1$ in shoe $K$, using circle information in Table \@ref(tab:resulttable). The blue circle in $Q$ and red circle in $K$ look pretty similar. We will repeat this process for two more circles that we fix in $Q$. The function `match_print` basically repeats this process for three circles in shoe $Q$. 


## Drawing Conclusions

To determine whether impressions $Q$ and $K$ were made by the same shoe or not, we need to define the signature of the questioned shoe outsole impression. We define the signature of shoe $Q$ as three circular areas in edge detection. By using edge detection of the outsole pattern in shoe $Q$, the signature will include class, subclass directly and existing RACs indirectly.
...
Figure \@ref(fig:signature) shows the signature in $Q$ and the corresponding areas in $K$ found by the `match_print()` function. We use the average of thefive similarity features from the three circle matchings to draw a conclusion. If the circles in $K$ match the circle in $Q$, then the geometry of the three circles in $Q$ and $K$ should be very similar. The blue lines  in Figure \@ref(fig:signature) form a triangle whose vertices are the centers of three circles in each shoe impression. The difference of the lengths of two triangles will become an additional feature used to determine if $Q$ and $K$ were made my the same shoe or not. If two triangles (blue) in Figure \@ref(fig:signature) have similar lengths, this information can indicate one additinal evidence that shoe $Q$ and shoe $K$ are from the same source. 


```{r finaltable, results='asis', echo=FALSE}
X1<-c('q1-$k^*_1$',18, 12.05, 0.75, 0.97, 0.3, 0.58)
X2<-c('q2-k^*_2',17, 10.57, 0.53, 0.91, 0.43, 0.55)
X3<-c('q3-k^*_3',20, 12.14, 0.63, 1.00, 0.24, 1.03)
...
Figure \@ref(fig:eximgs) displays three outsole impressions. Impression $Q$ is from the same  source as either $K_1$ or $K_2$. The two known impressions are from the same brand and model of shoes, which were worn by two different people for about six months. Since the three impressions all share class and subclass characteristics, it is a very hard comparison. There are some differences among images $Q, K_1, K_2$ but it is hard to determine if these differences due to measurement errors or wear and tear of the outsole or RACs.  
...
```
Here, we draw the graph of $Q$ and $K_1$ with circles in $Q$. 

Next, we run `match_print()` to findcircles in $K$ that most closely correspond to the circles in $Q$. The result is shown in Figure \@ref(fig:exKM). In $K_1$, the algorithm finds the closest circles which show the most overlap with each of the circles that we fixed in $Q$. 
...
Table \@ref(tab:KMtable1) and Table \@ref(tab:KMtable2) are output tables from `match_print()`. Table \@ref(tab:KMtable1) is the information of center and radius for three circles that showed the best overlap with three circles in $Q$. Table \@ref(tab:KMtable2) shows the similarity features between each circle in $Q$ and the corresponding circle in $K_1$. Each row in Table  \@ref(tab:KMtable2) shows the similarity features from circle $q_i$ and circle $k^{*}_i$ when $i=1,2,3$. 



### Comparison $Q$ vs. $K_2$

To compare $Q$ and $K_2$, we selected the same three circles in $Q$ and ask the `match_print()` function to find the best matching circles in $K_2$.  

```{r matchKM2, eval=FALSE}
match_print(imgQ, imgK2, input_circles_Q)
```
...
Figure \@ref(fig:exKNM) shows the final matching result. In this comparison, green circle in $K_2$ is in a very different position compared to the position of the green circle in $Q$. 
...
Table \@ref(tab:KNMtable1) and Table \@ref(tab:KNMtable1) show the location information of matching circles in $K$ for fixed circle in $Q$ and the corresponding similarity features, respectively. 


### Interpretation of comparisons of $Q, K_1$ and $Q, K_2$ 

To summarize similarity features from the match of the two impressions, we will take the average of each similarity feature across the three circle matchings, except rotation angle estimations. For features such as clique size and overlap, larger values indicate mated impressions. For the features median distance and absolute value of differences of lengths from two triangles in two impressions, lowever values indicate mated impressions. For the rotation angle estimation, we use the standard deviation of the three values because we are interested in comparing the three values to each other, not in the value of the angles themselves. If all rotations are about the same, the algorithm is consistently finding those patterns, and the prints are likely from the same shoe, even if the image has been rotated . If the rotation angles are very different, however, the algorithm is forcing similarity where there probably isn't any, and the prints are likely from different shoes. 
