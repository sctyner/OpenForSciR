...
We will now walk through the process of performing a bullet match. We will take three bullets, two of which were in fact fired from the same gun barrel, and one which was fired from a different gun barrel. Without that knowledge, we will see what the algorithm suggests is the probability of each occurrence.
...
The role of human decisions in forensic science is perhaps most studied in the fingerprint domain, which will be the focus of this chapter. High-profile examples of misidentification (e.g. Madrid train bomber, etc.) have led to studies showing that fingerprint examiners, like all humans, may be susceptible to biased instructions and unreliable in final decisions [@dror2008meta] or influenced by external factors or contextual information [@dror2006, @dror2010vision]. These examples of mistakes highlight the fact that fingerprint analysis is far from error-free, and stand in contrast to common perceptions of the accuracy of fingerprint examination. 

The FBI "Black Box" study [@ulery2011] was the first large-scale study performed to assess the accuracy and reliability of latent print examiners' decisions.  The questions were designed to include a range of attributes and quality seen in casework, and to be representative of searches from an automated fingerprint identification system. The overall false positive rate in the study was $0.1\%$ and the overall false negative rate was $7.5\%$. These computed quantities, however, have excluded all "inconclusive" responses (i.e. neither identifications nor exclusions). This is noteworthy, as nearly a third of all responses were inconclusive and respondents varied on how often they reported inconclusives. Respondents who report a large number of inconclusives, and only make identification or exclusion decisions for the most pristine prints, will therefore have substantially lower false positive and false negative rates than respondents who reported fewer inconclusives. The authors of the study also note that it is difficult to compare the error rates and inconclusive rates of individual examiners because each examiner saw a different set of fingerprint images (@ulery2011 Appendix 3). In other words, it would be unfair to compare the error rate of someone who was given a set of "easy" prints to compare to the error rate of someone who was given a set of "difficult" prints to compare. A better measure of examiner skill would account for both error rates and difficulty of prints that were examined. 

There are many additional areas in the forensic domain that rely on human decision-making. Firearms examination (See, e.g., @nas2009 pg. 150-155, OTHER CHAPTERS for overview) is similar to latent print examination in many ways, particularly in that examiners rely on pattern evidence to determine whether two cartridges originated from the same source. Handwriting comparison (See CHAPTER for introduction, @nas2009 pg. 163-167 on `Questioned Document Examination' and @stoelshaky for discussion) consists of examiners determining whether two smples of handwriting were authored by the same person, taking potential forgery or disguise into account. A third example is interpreting mixtures of DNA evidence (CHAPTER 4, @pcast Section 5.2). A DNA mixture is a biological sample that contains DNA from two or more donors and requires analysts to make subjective decisions to determine how many individuals contributed to the DNA profile. 

The main difference in forensic decision-making processes that we will consider here is in the examiners' underlying proficiency, although other differences exist as well. Accurately measuring proficiency, or examiner skill, is valuable not just for determining whether a forensic examiner has met baseline competency requirements, but for training purposes as well. Additionally, if proficiency is not accounted for among a group of participants, which often include trainees or non-experts as well as experienced examiners, the overall results may be biased. 

There also exist differences among the forensic evaluation tasks themselves. Properties of the evidence, such as the quality, quantity, concentration, or rarity of characteristics may make it easier or harder to evaluate. Some pieces of evidence, regardless of how skilled the examiner is, will not have enough information to result in an identification or exclusion in a comparison task. An inconclusive response, in this case, should be treated differently than an inconclusive response for a more straightforward comparison. 
...
*TODO still needs more filling out*

The case study below relies on the following software packages: 

- `Stan`: estimate parameters for the model using MCMC 
- `rstan`: call Stan, a program for MCMC estimation, from R 
- `blackboxstudyR`: provides functions for working with the black box data, implementations of basic IRT models in Stan, and plotting functions for results. Main Functions: 
...
*TODO talk with Sam more about this*

An IRT analysis produces estimates of both participant proficiency and item difficulty. A major benefit of using an IRT analysis, as opposed to the raw scores alone, is that the proficiency estimates account for the difficulty of items, and vice-versa. As mentioned previously, this property is especially useful for settings where participants respond to different subsets of items. 

By comparing the estimated proficiency to more traditional measures of participant performance (e.g. false positive rate or false negative rate), we can see whether there are aspects captured by proficiency that are not captured in other measures. For instance, the false positive rate and the false negative rate contain no information about the inconclusive rate. 

In the forensic setting, completing an IRT analysis will often include an additional step of choosing how the data should be scored. A further type of conclusion we may wish to draw is which scoring scheme is most appropriate for the setting at hand. In some cases, the optimal scoring scheme may be determined using expert knowledge, or by specifying particularly which conclusions are/are not acceptable for each item. In other cases, it may not be possible to determine the optimal scoring scheme before fitting an IRT model. In those cases, multiple scoring methods should be used to fit an IRT model, and the results from each model should be compared and contrasted. 
...
We can get a summary of the black box data (`TestResponses`) using the `str` function:

```{r, echo = TRUE, eval = FALSE}
str(TestResponses)
```

`TestResponses` is a data frame in which each row corresponds to an examiner $\times$ item response. In addition to the examiner ID (`Examiner_ID`) and item ID (`Pair_ID`), the data contains:
...
Inconclusive responses complicate the scoring of the exam due to both their ambiguity and prevalence. "Inconclusive" is never keyed as the correct response by the FBI, which leads to the ambiguity in scoring. There are a large number of inconclusive answers (`r sum(TestResponses$Compare_Value == "Inconclusive", na.rm = TRUE)` of `r nrow(TestResponses)` responses), and examiners vary on which latent print pairs are inconclusive. 

This package includes five methods to score inconclusive responses:
...
We'll proceed with an IRT analysis of the data under the `inconclusive_mcar` scoring scheme, which is how the data were scored under Ulery et al (2011). 
...
In practice, it is necessary to ensure that the MCMC sampler has converged using a variety of diagnostics. We omit these steps here for brevity, but they can be found in a separate package vignette. 

After the model has been fit, we can plot the posterior distributions of difficulties and proficiencies:

```{r, echo = TRUE}
plot_difficulty_posteriors(ii_model) -> p1
plot_proficiency_posteriors(ii_model) -> p2
grid.arrange(p1, p2, ncol = 2)
```

The lighter gray represents the 95\% posterior interval and the black interval represents the 50\% posterior interval. If we examine the posterior intervals for the difficulty estimates ($b$, left side plot), we can see three distinct groups which have noticeably larger intervals, and thus more uncertainty regarding the estimate: 1) those on the bottom left, 2) those on the upper right, and 3) those in the middle. To understand why this is happening, we plot the estimated difficulties against the observed score for each question.

```{r diff-observed, fig.cap="Difficulty vs Observed Item Score", message = FALSE, warning = FALSE}
obs_score = bb_item_score(TestResponses, im_scored)

# change this plot to a function

mcmc_intervals_data(as.array(im_model),
                      regex_pars = 'b',
                      prob_outer = .95) %>%
  mutate(., qID = 1:745) %>% # Don't hard code this when changing to a function
  right_join(., obs_score, by = "qID") %>% 
    ggplot(., aes(
      x = score,
      y = m,
      ymin = ll,
    ...
    labs(x = "Observed Score", y = "Difficulty Estimate") +
  my_theme -> p1
...
mcmc_intervals_data(as.array(im_model),
  ...
  filter(., is.na(score)) %>%
    ...
    labs(x = "Observed Score = NA", y = "Difficulty Estimate") +
  my_theme -> p2

grid.arrange(p1, p2, ncol = 2, widths = c(3,1))
```

This plot confirms three points: 1) In general, the difficulty estimate from the model correlates with the observed score under the appropriate scoring scheme, 2) the large uncertainty intervals found at the lower and upper ends of the difficulty range are for questions that everybody got right (i.e. observed score = 1) or wrong (i.e. observed score = 0), and 3) the large uncertainty intervals found in the middle of the range are for questions that nobody answered (i.e. observed score = NA, right side plot). 

### IRT accounts for different question sets

As discussed above, it is problematic to compare the error rates across participants in the Black Box Study due to participants seeing different sets of questions. It would thus be "unfair" to compare the false positive error rate of a participant who saw a hard set of questions to the same error rate of a participant who saw a very easy set of questions. A benefit of IRT analysis is that the difficulty of questions is accounted for when estimating participant proficiency. 

As expected, the estimated proficiencies correlate with the observed score (Figure \@ref(fig:prof-observed). That is, participants with a higher observed score are generally given larger proficiency estimates than participants with lower scores. There are, however, cases where participants scored roughly the same on the study but are given vastly different proficiency estimates. For example, the highlighted participants in the right-side plot above all scored between 94\% and 96\%, but their estimated proficiencies range from $-1.25$ to $2.5$.  


```{r prof-observed, fig.cap="Proficiency vs Observed Score"}
...
mcmc_intervals_data(as.array(im_model),
                      regex_pars = 'theta',
                      prob_outer = .95) %>%
  mutate(., exID = 1:169) %>% # Don't hard code this when changing to a function
...
```{r prof-by-diff, fig.cap="Proficiency vs Average Question Difficulty, for participants with observed score between 94 and 96 percent correct."}
q_diff = apply(rstan::extract(im_model, pars = "b")$b, 2, median)
qID = as.numeric(fbi$Pair_ID)
exID = as.numeric(fbi$Examiner_ID)
q_diff = q_diff[qID]
is.skipped = fbi$Latent_Value == 'NV' | fbi$Compare_Value == 'Inconclusive'
false.pos = fbi$Compare_Value == 'Individualization' & fbi$Mating == 'Non-mates'
false.neg = fbi$Compare_Value == 'Exclusion' & fbi$Mating == 'Mates'
...
mcmc_intervals_data(as.array(im_model),
  ...
  mutate(., exID = 1:169) %>% # Don't hard code this when changing to a function
  right_join(., obs_p_score, by = "exID") %>% 
  full_join(., ex_resp_data, by = "exID") %>%
...
### IRT complements error rate analysis

The original analysis of the FBI "Black Box" Study (Ulery et al 2011) did not include analysis of participant error rates, since each participant saw a different question set. Since proficiency accounts for the diffculty of question sets, we can directly compare participant proficiencies to each other, and also see how error rates and proficiency are related. 
...
```{r prof-error-rate, fig.cap="Proficiency vs False Positive Rate (left) and False Negative Rate (right)"}
mcmc_intervals_data(as.array(im_model),
                      regex_pars = 'theta',
                      prob_outer = .95) %>%
  mutate(., exID = 1:169) %>% # Don't hard code this when changing to a function
...
---
# Obtaining measures of uncertainty for the difficulty estimates is a further benefit of using an Item Response Theory-based approach, rather than using observed false positive and false negative rates. We now plot the difficulty (Figure \ref{fig:diff-observed}), but highlighting the item difficulty estimates that are significantly different from one another, using a cutoff of $-2$. This is because IRT accounts for both the amount of data that is present for each question and the proficiency of participants who answered each question. 
...
We can examine the proficiency estimates and observed scores for each participant under each of the three scoring schemes, similar to Figure \@ref(fig:prof-observed) above. 

```{r prof-three-scores, fig.cap="Proficiency vs Observed Score for each of three scoring schemes"}
...
mcmc_intervals_data(as.array(nci_model),
                      regex_pars = 'theta',
                      prob_outer = .95) %>%
  mutate(., exID = 1:169) %>% # Don't hard code this when changing to a function
...
mcmc_intervals_data(as.array(pc_model),
                      regex_pars = 'theta',
                      prob_outer = .95) %>%
  mutate(., exID = 1:169) %>% # Don't hard code this when changing to a function
...
```{r prof-three-scores-mastery, fig.cap="Proficiency vs Observed Score for participants who did not achieve mastery under each of three scoring schemes." }
p_score_im %>%
  bind_rows(., p_score_nci) %>%
  bind_rows(., p_score_pc) %>%
...
The number of examiners that have not demonstrated mastery varies based on the scoring method used (11 for "nci", 8 for "pc" and 11 for "im") due to the variation in range of proficiency estimates. Additionally, for each of the scoring schemes, there are a number of examiners that did achieve mastery with the same observed score as those that did not demonstrate mastery. This is due to a main feature of item response models discussed earlier: examiners that answered more difficult questions are given higher proficiency estimates than those that answered easier questions.

We've also drawn dotted lines between proficiency estimates that correspond to the same person. Note that many of the participants who do not achieve mastery under one scoring scheme _do_ achieve mastery under the other scoring schemes, as evidenced by a lack of a dotted line. There are also a few participants who do not achieve mastery under any of the scoring schemes. This raises the question of how much the proficiency estimates change for each participant under the different scoring schemes. 

The plot on the left below shows both a change in examiner proficiencies across scoring schemes (i.e. the lines connecting the proficiencies are not horizontal) as well as a change in the ordering of examiner proficiencies (i.e. the lines cross one another). That is, different scoring schemes affect examiner proficiencies in different ways. 

The plot on the right illustrates two groups that see substantial differences in their proficiency estimates. Examiners 105 and 3 benefit from the leniency in scoring when inconclusives are treated as missing. When inconclusives are scored as incorrect ("nci") or partial credit ("pc"), they see a substantial decrease in their score due to both reporting a high number of inconclusives and differing from other examiners in their reasoning for reporting inconclusives. Examiners 142, 60 and 110, on the other hand, are hurt by the leniency in scoring when inconclusives are treated as missing. These examiners benefit from inconclusives being scored as correct when they match the consensus reason ("nci") or being worth partial credit ("pc"). 
...
p_score_im %>%
  ...
  labs(x = "Scoring Method",
       y = "Estimated Proficiency") +
  gghighlight( max.diff > 1.95, label_key = id) +
  my_theme-> p2
