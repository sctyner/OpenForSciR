...
glass_log_long %>% filter(element %in% c("Na","Ti","Zr","Hf")) %>% 
  ggplot() + 
  #geom_rect(data = rect,inherit.aes=F, aes(xmin = xmin, xmax = xmax, ymin = ymin, ymax = ymax), fill = "grey80", alpha = .2) + 
  #annotate("rect", xmin = 17, xmax = Inf, ymin = -Inf, ymax = Inf, fill = "grey80", alpha = .2) + 
  geom_boxplot(aes(x = pane, y = ppm, fill = mfr), alpha = .8, outlier.size = .5, size = .1) + 
  scale_fill_manual(name = "Manufacturer", values = cols) +
  facet_wrap(~element, scales = "free", nrow = 2, labeller = label_both) + 
  theme(legend.position = "none") + 
  scale_x_discrete(labels = c("AA", rep("", 30), "BA", rep("", 15), "BR")) + 
  labs(x = "Pane (in order of manufacture)", y = "Concentration (ppm)")
...
down_diff_Q_K_data2<-downSample(diff_Q_K_data[,c(1:18)], diff_Q_K_data$class)
head(down_diff_Q_K_data2)
names(down_diff_Q_K_data2)[19]<-"class"
down_diff_Q_K_data2$class<-as.factor(down_diff_Q_K_data2$class)
prob_prediction2 <- predict(RF_classifier, down_diff_Q_K_data2, type="prob")[,"KM"]  
head(prob_prediction2)

down_diff_Q_K_data2$prob<-prob_prediction2
