...
There could be two questions of interest in glass fragments comparison; (1) The specific source problem, whether the questioned fragment ($Q$) is from the glass pane that known fragments ($K$) found at the crime scene, (2) the common source problem, whether the questioned fragment ($Q$) shares the source that known fragments belong to.  Regardless of which question we are facing, the statistic we need to help answering the question is the similarity between $Q$ and $K$. There are lots of ways to define the similarity between two glass fragments, $Q$ and $K$, but the similarity should be defined based on the what types of data or measurements we have for glass fragments. For example, if we have elemental compositions measured in ppm as numerical values, the easy version of similarity would be the differneces of chemical compositions between $Q$ and $K$. 
