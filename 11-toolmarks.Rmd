...
The 'ID' variable is a toolmark identifier and Factor variable. The side of the screwdriver from which the marking was made is called 'side'. It is a factor variable with values A or B. The 'angle' variable is the degree of the angle under which the toolmark was created. 'Rep' on the other hand shows the replicate number for a toolmark. The sample data provided is not complete, therefore it only shows one randomly sampled replicate for each of the 16 profiles under consideration. For a more complete data set or more information please contact the references given below.
...
The R functions 'fixed_width_with_model' and 'fixed_width_with_no_model' use the diamond search fixed width method in its computations [@hadlermorris]. These functions return results in the form of variables and plots. The meaning of these lists, variables and plots are given below:
...
9.  mark2_decompostion =  Plot of decomposition, shows the class and individual component of the second toolmark along with the normalized toolmark.
