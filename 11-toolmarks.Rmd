...
### Select values for the coarseness parameter.
For the Diamond Search fixed width functions there are two kinds smoothness parameter values namely 'coarse' and 'fine'. The coarse parameter is used for normalization and getting the class component, while the parameter 'fine' is used to get the individual component of the two markings. The default value for 'coarse' is defined as 0.25 while that for the 'fine' is 0.01.
...
```{r}
# Show original marking picture
# Show normalized marking picture
